...
> **Изменения v1.2 относительно v1.1:** открыт скоуп v2 — добавлена первая фича v2 **FR-14** (деплой стека из compose-файла через UI: загрузка YAML + env, хранение в БД, разворачивание сети/секретов/сервисов, валидация, prune с подтверждением). Остальные v2-пункты (Telegram-нотификации, роли пользователей, exec-терминал) остаются впереди.
>
...
### FR-14. Деплой стека из compose-файла (первая фича v2)

В v1 стек — только агрегация живых сервисов по label (FR-08). FR-14 добавляет
разворачивание стека **из файла** — то, что делает `docker stack deploy -c file.yml`.

- 3.14.1. **Загрузка стека:** пользователь вставляет compose/stack YAML (или
  загружает `.yml`-файл в то же поле). Поддержка version 3.x, YAML-якорей
  (`&anchor`/`*ref`), интерполяции `${VAR}` и `${VAR:-default}`.
- 3.14.2. **Переменные окружения:** отдельное поле `.env` (текст, формат
  `KEY=value`) плюс ручные пары ключ-значение в UI. Env применяется при
  интерполяции YAML. Значения хранятся **зашифрованными** (AES-GCM, ключ
  `DSMS_ENCRYPTION_KEY`, как FR-13) — в env бывают секреты (`JWT_SECRET`, пароли БД).
- 3.14.3. **Хранение в БД:** таблица **stacks** (`name`, `compose_yaml`,
  `env_enc`, timestamps, `user_id`). Панель помнит файл и env — стек можно
  открыть, отредактировать и передеплоить. `GET /stacks` объединяет
  **managed** (из таблицы, с файлом) и **discovered** (агрегация по label, v1).
- 3.14.4. **Валидация до деплоя:** dry-run парсинг — сколько сервисов/сетей/
  секретов будет создано, список ошибок. Ничего не создаёт.
- 3.14.5. **Оркестрация деплоя:** порядок — сети (non-external overlay) →
  secrets/configs из файла → сервисы. Каждый сервис получает имя `<stack>_<svc>`
  и label `com.docker.stack.namespace=<stack>`. Существующий сервис —
  `ServiceUpdate`, новый — `ServiceCreate`. Панель **не навязывает**
  `UpdateConfig.Order` (уважает то, что в файле — урок из FR-08). Ошибка одного
  сервиса не роняет весь деплой; возвращается сводка (создано/обновлено/упало).
- 3.14.6. **Prune при передеплое:** сервисы стека, которых больше нет в файле,
  удаляются — **только с явным подтверждением в UI** (по умолчанию выключено,
  защита от случайного сноса).
- 3.14.7. **Безопасность:** yaml/env не пишутся в audit (только имя стека +
  сводка) и не логируются; env_enc не отдаётся клиенту в открытом виде.
- 3.14.8. **Конвертация compose → swarm-спека** выполняется собственным
  конвертером панели (контроль размера образа ≤25 МБ и поведения); покрываются
  поля: image, command, environment, `deploy` (replicas, placement.constraints,
  resources, restart_policy, update_config), networks, extra_hosts, ports
  (включая `mode: host`), secrets, configs, healthcheck, labels.
...
| GET | `/stacks` | список стеков + агрегированный статус (FR-08); managed+discovered (FR-14) |
...
| POST | `/stacks/validate` | `{compose_yaml, env}` — dry-run парсинг, превью/ошибки (FR-14) |
| POST | `/stacks` | `{name, compose_yaml, env}` — сохранить в БД + задеплоить (FR-14) |
| GET | `/stacks/{name}/source` | сохранённый yaml+env для редактирования (FR-14) |
| PUT | `/stacks/{name}` | обновить yaml/env + передеплой (diff, опц. prune) (FR-14) |
...
| docker.sock у агента = root каждой ноды (нужен для FR-10/FR-11) | API агента слушает только внутри overlay-сети, X-Agent-Token (shared secret), только точечные операции (df/volumes/prune) — произвольные вызовы Docker API не проксируются |
...
- ~~Деплой стеков из UI: загрузка/редактирование compose-файла~~ → **перенесено в реализацию как FR-14** (см. раздел 3), этап 9 (`docs/plans/stage-9-stack-deploy.md`)
- Управление несколькими кластерами из одной панели
