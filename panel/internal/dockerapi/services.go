...
// реестре — аналог `docker service update --image repo:tag`. Старые задачи
// заменяются согласно UpdateConfig сервиса (order/parallelism из compose;
// панель его не переопределяет).
